...
router.post('/register', async (request, response) => {
    ...
    const checkExist = await User.findOne({ email: request.body.username });
...
router.post('/login', async (request, response) => {
    ...
    if (!user) return response.status(422).send('Username or Password is not correct');

    const checkPassword = await bcrypt.compare(request.body.password, user.password);

    if (!checkPassword) return response.status(422).send('Username or Password is not correct');
